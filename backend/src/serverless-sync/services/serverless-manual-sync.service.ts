...
/**
 * 수동 동기화 재시도 설정
 * - Serverless 환경에서는 타임아웃이 있으므로 더 짧은 지연과 적은 재시도 사용
 */
...
/**
 * Serverless 수동 동기화 서비스
 * - BullMQ/Redis 없이 직접 동기화 수행
 * - Vercel Serverless Functions에서 사용
 * - Rate Limit 처리 및 재시도 로직 포함
 */
@Injectable()
export class ServerlessManualSyncService {
  private readonly logger = new Logger(ServerlessManualSyncService.name);
  ...
  async executeManualSync(dto: ManualSyncDto): Promise<ManualSyncResult> {
    ...
    // 이미 진행 중인 동기화가 있으면 해당 Promise 반환 (중복 방지)
  ...
  private async executeManualSyncInternal(
    ...
    this.logger.log(
      `[Serverless] Starting manual sync: direction=${direction}, taskId=${dto.taskId ?? 'all'}`,
    );
    ...
    this.logger.log(
      `[Serverless] Manual sync completed: success=${result.successCount}, failed=${result.failedCount}, skipped=${result.skippedCount}, duration=${result.durationMs}ms`,
    );
  ...
  private async createNotionPageWithRollback(
    ...
    if (updateError) {
      // 보상 트랜잭션 - DB 저장 실패 시 Notion 페이지 아카이브
      this.logger.error(
